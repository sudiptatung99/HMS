...
<x-app-layout> 
    <div class="container-fluid">
        <div class="nk-block-between mb-3">    
            ...
            <a href="{{ route('enquiry.create') }}" class="btn btn-white btn-outline-light">   
                        ...
                        <table class="datatable-init-export nowrap table">  
                            ...
                            @foreach($enquiry as $key => $enquiry) 
                                <tr>
                                    <td>{{ $no++ }}</td>
                                    <td>{{ $enquiry->name }}</td>
                                    <td>{{ env('COUNTRY_CODE') }} {{ $enquiry->contact }}</td>
                                    <td>{{ $enquiry->enquiry_type }}</td>
                                    <td>{{ $enquiry->description }}</td>
                                    <td>
                                        @if($enquiry->status == 'Solved') 
                                            <span class="badge badge-dot bg-success">{{ $enquiry->status }}</span>
                                        @elseif($enquiry->status == 'Pending')  
                                            <span class="badge badge-dot bg-danger">{{ $enquiry->status }}</span>   
                                        @endif 
                                    </td>  
                                    <td> 
                                        <div class="drodown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end" data-popper-placement="bottom-end">  
                                                ...
                                                @can('update-enquiry')     
                                                    <li>
                                                        <a href="{{ route('enquiry.update', encrypt($enquiry->id)) }}">
                                                ...
                                                @can('delete-enquiry')      
                                                    <li>
                                                        <a href="#" data-bs-toggle="modal" data-bs-target="#deleteWarning{{ $enquiry->id }}">
                                ...
                                <div class="modal fade" id="deleteWarning{{ $enquiry->id }}" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">   
                                    <div class="modal-dialog modal-dialog-centered">     
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                <a class="btn btn-danger" href="{{ route('enquiry.delete', encrypt($enquiry->id)) }}">Yes</a>     
