@section('title') {{ 'Radiology Parameter Update' }} @endsection
<x-app-layout>
            ...
            <div class="col-xl-12">
                <div class="card">  
                    <div class="card-body mt-3"> 
                        <form action="{{ route('radiology-parameter.update.store', encrypt($radiology_parameter->id)) }}" method="POST">         
                        @csrf     
                            <div class="row mb-4"> 
                                <label class="col-sm-2 col-form-label">Name <i class="text-danger">*</i></label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" name="name" value="{{ $radiology_parameter->name }}" required>
                                </div>
                            </div>     
                            <div class="row mb-4">
                                <label class="col-sm-2 col-form-label">Range <i class="text-danger">*</i></label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" name="range" value="{{ $radiology_parameter->range }}" required>
                                </div> 
                            </div>       
                            <div class="row mb-4"> 
                                ...
                                <div class="col-sm-8"> 
                                    <select class="form-select js-select2" data-search="on" name="unit" required>   
                                        {{ getTestUnit($radiology_parameter->unit) }}     
                                    </select>    
                                </div> 
                            </div>   
                            <div class="row mb-4">
                                ...
                                <div class="col-sm-8">  
                                    <textarea class="form-control" name="description" required>{{ $radiology_parameter->description }}</textarea>   
                                </div>   
                            </div>     
                            <div class="row mb-4">
